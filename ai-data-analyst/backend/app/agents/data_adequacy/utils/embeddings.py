...
try:
    ...
    # Fallback/Mock if Pinecone v3 not installed, though it should be
    Pinecone = None
        ...
        for i in range(0, len(texts), batch_size):
            ...
            for j, result in enumerate(batch_results):
                if isinstance(result, Exception):
                    final_batch_results.append(EmbeddingResult(
                        success=False,
...
class PineconeManager:
    ...
    def _initialize_pinecone(self):
        """Initialize Pinecone client and index (SDK v3+)."""
        try:
            if not self.api_key:
            ...
            if Pinecone:
                ...
                if self.index_name not in existing_indexes:
                    ...
                    # Assumption: Using ServerlessSpec for new standard, or PodSpec if configured.
                    # For simplicity, if env is meant for pods, we need PodSpec.
                    # But config.PINECONE_ENVIRONMENT might suggest pod usage (e.g. us-east-1).
                    # SDK v3 create_index syntax:
                    # pc.create_index(name, dimension, metric, spec)
                    
                    # We will attempt to create it but if specs are missing, it might fail.
                    # However, usually we expect index to exist or be created by infra.
                    # For now, let's skip auto-creation or do basic creation if supported.
                    # Since existing code used simple create_index, we'll try to replicate default behavior
                    # or just proceed if index exists.
                    pass 
            ...
            # Don't raise, allow running without Pinecone (checking success later)
    ...
    def query_similar(self, 
        ...
        if not self.index:
             return {"success": False, "error": "Pinecone index not initialized", "results": []}
