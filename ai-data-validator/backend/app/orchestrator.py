"""Orchestrator Agent - central brain coordinating all agents with ReAct loop."""
...
class OrchestratorAgent:
    """
    Interactive Orchestrator Agent (IOA) - Central coordination system.
...
async def start_validation(user_goal: str, domain: str = "general", files: List[str] = None) -> Dict[str, Any]:
    ...
    return await orchestrator.run_validation(user_goal, domain, files)


async def continue_session_validation(orchestrator: OrchestratorAgent, user_answers: Dict[str, str]) -> Dict[str, Any]:
    """Continue an existing validation session."""
    return await orchestrator.continue_validation(user_answers)
